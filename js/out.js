(() => {
  ...
  var api = "/www/canals5/phox/api";
  var photos = "/photos/";
  var api_photos = api + photos;

  // module/photoloader.js
  var loadResource = (uri) => {
    return fetch(url + uri, { credentials: "include" }).then((res) => {
      if (res.ok) {
        return res.json();
      }
  ...
  var page = 1;
  var load = () => {
    return loadResource(api_photos + `?size=9&page=${page}`);
  };
  var prev = () => {
    page--;
    return load();
  ...
  // module/photo.js
  var getPicture = (id) => {
    let img = loadResource(api_photos + id);
    displayPicture(img);
    displayCategorie(getCategorie(img));
    displayCommentaires(getCommentaires(img));
  };
  var getCategorie = (img) => {
    return img.then((data) => {
      return loadResource(data.links.categorie.href);
    });
  };
  var getCommentaires = (img) => {
    return img.then((data) => {
  ...
  // module/ui.js
  var template_photo = import_handlebars.default.compile(document.querySelector("#photoTemplate").innerHTML);
  var template_categorie = import_handlebars.default.compile(document.querySelector("#categorieTemplate").innerHTML);
  var template_commentaires = import_handlebars.default.compile(document.querySelector("#commentairesTemplate").innerHTML);
  var template_galerie = import_handlebars.default.compile(document.querySelector("#galerieTemplate").innerHTML);
  var la_photo = document.querySelector("#la_photo");
  var la_categorie = document.querySelector("#la_categorie");
  var les_commentaires = document.querySelector("#les_commentaires");
  var la_galerie = document.querySelector("#la_galerie");
  var displayPicture = (img) => {
    img.then((data) => {
      la_photo.innerHTML = template_photo({ url: url + data.photo.url.href, photo: data.photo });
    });
  };
  var displayCategorie = (categ) => {
    categ.then((data) => {
      la_categorie.innerHTML = template_categorie({ categorie: data.categorie.nom });
    });
  };
  var displayCommentaires = (com) => {
    com.then((data) => {
      les_commentaires.innerHTML = template_commentaires({ commentaires: data.comments });
    });
  };
  var displayGalerie = (gal) => {
    gal.then((data) => {
      let prev2 = page > 1;
      let next2 = data.count > 9 * page;
      data.photos.forEach((elem) => {
        elem.photo.thumbnail.href = url + elem.photo.thumbnail.href;
      });
      la_galerie.innerHTML = template_galerie({ photos: data.photos, prev: prev2, next: next2 });
      data.photos.forEach((elem) => {
        elem.photo.thumbnail.href = url + elem.photo.thumbnail.href;
      });
      let index = 0;
